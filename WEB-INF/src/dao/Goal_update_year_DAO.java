...
public class Goal_update_year_DAO extends DAO{
    public  Boolean goal_year_parent_rename(String goal_year_parent_id,String goal_rename){//親目標名更新
    ...
    public  Boolean goal_year_child_rename(String goal_year_child_id,String goal_rename,String goal_year_parent_id){//子目標名更新
        Connection con = getConnection(); //DBに接続
        PreparedStatement st;
    try{
    ...
    public  Boolean goal_year_parent_add(String goal_id,String user_id,String goal_name){//親目標追加
        Connection con = getConnection(); //DBに接続
        PreparedStatement st;
    ...
    try{
        ...
        st.close();
        con.setAutoCommit(false);
        st2=con.prepareStatement("insert into goal_year_parent values(?,?,?,?,null)");
    ...
    public  Boolean goal_year_child_add(String goal_id,String goal_year_parent_id,String user_id,String goal_name){//子目標追加
        Connection con = getConnection(); //DBに接続
        PreparedStatement st;
        PreparedStatement st2;
        Goal g=new Goal();
    try{
        ...
        st.close();
        con.setAutoCommit(false);
        st2=con.prepareStatement("insert into goal_year_child values(?,?,?,?,?,null)");
    ...
    public  Boolean goal_achieve_update(String goal_year_child_id){//子目標進捗更新
        Connection con = getConnection(); //DBに接続
        PreparedStatement st;
    try{
        st=con.prepareStatement("update goal_year_child set achieved_goal = 1 where child_id =?");
        st.setString(1,goal_year_child_id);
        int rs_update = st.executeUpdate();//sql文実施
    ...
    public  Boolean goal_achieve_parent_update(String goal_year_child_id){//子目標が全て達成した場合、親目標達処理する
        Connection con = getConnection(); //DBに接続
